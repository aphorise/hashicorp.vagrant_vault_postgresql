...
VV1=''  # VV1='VAULT_VERSION='+'1.5.8+ent'  # // to Install enterprise or other specific versions.
sVUSER='vagrant'  # // vagrant user
sHOME="/home/#{sVUSER}"  # // home path for vagrant user
sIP_CLASS_D='192.168.178'  # // NETWORK CIDR for configs
sIP="#{sIP_CLASS_D}.190"
	...
	(1..2).each do |iX|
		...
		if iX == 2 then
			config.vm.define vm_name="vault#{iX-1}" do |vault_node|
				...
				vault_node.vm.provision "shell", inline: "/bin/bash -c '#{VV1} PG_FQDN=#{sIP} #{sHOME}/install_vault_postgresql.sh'"
			end
		end
	end
