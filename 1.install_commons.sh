...
sudo apt-get -yqq update 2>&1>/dev/null && sudo apt-get install -yqq unzip curl htop screen jq coreutils systemd gnupg2 2>&1>/dev/null;
# // Ubuntu needs extras like: realpath & systemd as they are not shipped with defaults like in Debian
if [[ $(uname -ar) == *"Ubuntu"* ]] ; then sudo apt-get install -yqq realpath 2>&1>/dev/null ; fi ;
